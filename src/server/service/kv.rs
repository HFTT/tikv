...
pub struct Service<T: RaftStoreRouter<RocksEngine> + 'static, E: Engine, L: LockManager> {
    ...
    /// Used to report trace results generated by minitrace
    ...
    > Clone for Service<T, E, L>
{
    fn clone(&self) -> Self {
        Service {
            ...
            trace_reporter: self.trace_reporter.clone(),
        }
    }
}
...
macro_rules! handle_request {
    ($fn_name: ident, $future_name: ident, $req_ty: ident, $resp_ty: ident) => {
        fn $fn_name(&mut self, ctx: RpcContext<'_>, mut req: $req_ty, sink: UnarySink<$resp_ty>) {
            ...
            let (scope, report_routine) = trace_and_report!($fn_name, self.trace_reporter, &mut req, $resp_ty);
            let _g = scope.enter();
    ...
    for Service<T, E, L>
{
    ...
    fn coprocessor(
        ...
        let begin_instant = Instant::now_coarse();
        let (scope, report_routine) = trace_and_report!(
            coprocessor,
            self.trace_reporter,
            &mut req,
            CoprocessorResponse
    ...
    fn read_index(
        ...
        if let Err(e) = self.ch.send_command(
            cmd,
            Callback::read(cb)
                .with_trace_scope(Scope::from_local_parent("read_index::RaftCmdRequest")),
        ) {
            // Retrun region error instead a gRPC error.
    ...
    fn batch_commands(
        ...
        let trace_reporter = self.trace_reporter.clone();
...
fn handle_batch_commands_request<E: Engine, L: LockManager>(
    ...
    macro_rules! handle_cmd {
        ($($cmd: ident, $future_fn: ident ( $($arg: expr),* ), $req_name: ident;)*) => {
            match req.cmd {
                None => {
                ...
                Some(batch_commands_request::request::Cmd::Get(mut req)) => {
                    let (scope, report_routine) = trace_and_report!(kv_get, trace_reporter, &mut req, GetResponse);

                    if batcher.as_mut().map_or(false, |req_batch| {
                        req_batch.maybe_commit(storage, tx);
                        req_batch.can_batch_get(&req)
                    }) {
                        batcher.as_mut().unwrap().add_get_request(req, id, scope, report_routine);
                    } else {
                        let begin_instant = Instant::now();
                        let _g = scope.enter();
                        let resp = future_get(storage, req);
                        let resp = resp.map_ok(move |mut resp| {
                                drop(scope);
                                report_routine(&mut resp);
                ...
                Some(batch_commands_request::request::Cmd::RawGet(mut req)) => {
                    let (scope, report_routine) = trace_and_report!(kv_raw_get, trace_reporter, &mut req, RawGetResponse);

                    if batcher.as_mut().map_or(false, |req_batch| {
                        req_batch.maybe_commit(storage, tx);
                        req_batch.can_batch_raw_get(&req)
                    }) {
                        batcher.as_mut().unwrap().add_raw_get_request(req, id, scope, report_routine);
                    } else {
                        let begin_instant = Instant::now();
                        let _g = scope.enter();
                ...
                $(Some(batch_commands_request::request::Cmd::$cmd(mut req)) => {
                    let begin_instant = Instant::now();
                    let (scope, report_routine) = trace_and_report!($req_name, trace_reporter, &mut req, concat_idents!($cmd, Response));
                    let _g = scope.enter();
                    ...
                    let resp = resp.map_ok(move |mut resp| {
                        ...
                        .map_err(|_| GRPC_MSG_FAIL_COUNTER.$req_name.inc());
                    response_batch_commands_request(id, resp, tx.clone(), begin_instant, GrpcTypeKind::$req_name);
                })*
